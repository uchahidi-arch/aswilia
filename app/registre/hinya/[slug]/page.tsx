...
export default function HinyaPage({
  params,
}: {
  ...
  const [iles, setIles]       = useState<string[]>([]);
  ...
  useEffect(() => {
    import('@/lib/supabase').then(async ({ supabase }) => {
      ...
      const resolvedClan = resolvedLabel.replace(/^(Hinya|Wa|Daho)\s+/i, '').trim();
      setClan(resolvedLabel);

      // 2. Charger les personnes du hinya
      ...
      setPersons(list);
      setIles([...new Set(list.map(p => p.ile).filter((i): i is NonNullable<typeof i> => Boolean(i)) as string[])].sort());
      setLoading(false);
    });
  }, [slug]);
  ...
  if (notFound || !clan) {
    return (
      <div className="view-section">
        <div className="empty-grid">Hinya introuvable.</div>
      </div>
    );
  }
    ...
    <div style={{ position: 'relative', flex: 1, width: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div className="view-section" style={{ flex: 1, background: 'transparent', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div className="explorer-topbar">
          ...
          <span className="bc-item-v2" style={{ color: 'var(--t2)', cursor: 'default' }}>Hinya</span>
          <span className="bc-sep-v2">›</span>
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '12px', color: 'var(--t3)', marginTop: '4px' }}>
                  Hinya · Lignée matrilinéaire
                </div>
              </div>
              <div style={{ display: 'flex', gap: '10px' }}>
                {[
                  { val: persons.length, label: 'personnes' },
                  { val: iles.length,    label: 'île' + (iles.length !== 1 ? 's' : '') },
                ].map(({ val, label }) => (
                  <div key={label} style={{
          ...
          {/* Grille PersonCard */}
          {persons.length === 0 ? (
            <div className="empty-grid">Aucune personne enregistrée pour ce hinya.</div>
          ) : (
            <div className="folder-grid">
              {persons.map(p => (
                <PersonCard
