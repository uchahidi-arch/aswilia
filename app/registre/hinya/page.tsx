...
export default function HinyaIndexPage() {
    ...
    <div style={{ position: 'relative', flex: 1, width: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div className="view-section" style={{ flex: 1, background: 'transparent', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div className="explorer-topbar">
          ...
          <span className="bc-current-v2">Par Hinya</span>
        </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          ...
          ) : hinyaGroups.length === 0 ? (
            <div className="empty-grid">Aucun hinya trouvé.</div>
          ) : (
            <div className="folder-grid">
              {hinyaGroups.map(({ label, count }) => (
                <div
                  ...
                  onClick={() => router.push(`/registre/hinya/${slugify(label)}`)}
                >
              ...
              <div
                className="exp-card fadein"
                onClick={() => router.push(`/registre/hinya/__sans__`)}
              >
