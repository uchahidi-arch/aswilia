...
export default function AjouterEnfantPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [daho, setDaho] = useState(mere?.daho || '');
  const [localite, setLocalite] = useState(mere?.localite || '');
  const [ile, setIle] = useState<IleComores | ''>(parent?.ile || '');
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div>
              ...
              <input
                ...
                value={daho}
                onChange={e => setDaho(e.target.value)}
                style={{
                  width: '100%',
                  padding: '10px 12px',
                  border: '1px solid #ddd',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
            ...
            <div>
              ...
              <select
                value={ile}
                onChange={e => setIle(e.target.value as IleComores | '')}
                style={{
                  width: '100%',
                  padding: '10px 12px',
                  border: '1px solid #ddd',
              ...
              >
                ...
                {iles.map(i => <option key={i} value={i}>{i}</option>)}
              </select>
            </div>
          </div>
