...
import type { IleComores } from '@/lib/types';
  ...
  async function save() {
    ...
    try {
      await updatePerson(id, {
        ...
        ile: (ile || null) as IleComores | null,
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '24px', maxWidth: 540, margin: '0 auto', width: '100%' }}>
      ...
      {advOpen && (
        <div className="form-adv open">
          ...
          <div className="f-row-2">
            <div>
              <label className="f-lbl">Daho</label>
              <input className="f-input" placeholder="Sangani…" value={daho} onChange={e => setDaho(e.target.value)} />
            </div>
            <div>
              <label className="f-lbl">Localité d'origine</label>
              <input className="f-input" placeholder="Mitsoudje…" value={localite} onChange={e => setLocalite(e.target.value)} />
            </div>
          </div>
          <div className="f-sec">Biographie</div>
          <div className="f-row-2">
            <div>
              <label className="f-lbl">Île</label>
              <select className="f-sel" value={ile} onChange={e => setIle(e.target.value)}>
                <option value="">— Choisir —</option>
                <option>Grande Comore (Ngazidja)</option>
                <option>Anjouan (Ndzwani)</option>
                <option>Mohéli (Mwali)</option>
                <option>Mayotte (Maore)</option>
              </select>
            </div>
            <div>
          ...
          <div className="f-row-2">
            ...
            <div>
              ...
              <input className="f-input" placeholder="Sultan, Cadi…" value={metier} onChange={e => setMetier(e.target.value)} />
            </div>
          </div>
          <div className="f-row">
