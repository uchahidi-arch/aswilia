...
export default function AuthModal({ open, initialTab = 'login', onClose }: AuthModalProps) {
  ...
  const [signupIle,      setSignupIle]      = useState('');
      ...
      <div className="modal" style={{ maxWidth: '400px' }}>
        ...
        {tab === 'signup' && (
          <div className="m-body">
            ...
            {/* Île */}
            <div className="f-row">
              <label className="f-lbl">Île d&apos;origine</label>
              <select className="f-sel" value={signupIle} onChange={e => setSignupIle(e.target.value)}>
                <option value="">— Choisir —</option>
                {ILES.map(ile => <option key={ile}>{ile}</option>)}
              </select>
            </div>
