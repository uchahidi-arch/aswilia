...
export function useDB() {
  ...
  const searchPersons = useCallback(async (q: string): Promise<Person[]> => {
    const { data } = await supabase.from('persons').select('*')
      .or(`prenom.ilike.%${q}%,nom.ilike.%${q}%,clan.ilike.%${q}%,localite.ilike.%${q}%,daho.ilike.%${q}%`)
      .neq('masque', true)
      .limit(300);
    return data || [];
  }, []);
  ...
  // Propage clan/localite/ile/daho/prefix_lignee de la mère à toute sa lignée matrilinéaire
  const propagateMaternalLineage = useCallback(async (mereId: string, fields: Pick<Person, 'clan' | 'localite' | 'ile' | 'daho' | 'prefix_lignee'>): Promise<void> => {
  ...
  const exportJSON = useCallback(() => {
    ...
    a.download = `Aswilia_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  const exportGEDCOM = useCallback(() => {
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `Aswilia_${new Date().toISOString().slice(0, 10)}.ged`;
    a.click();
