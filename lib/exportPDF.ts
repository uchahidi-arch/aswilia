...
function buildCardHTML(person: Person | null, roleColor: string): string {
  ...
  if (person.clan)     meta += `<div style="font-size:7.5px;color:#78716C;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${person.prefix_lignee || 'Hinya'} ${person.clan}</div>`;
...
export async function exportPersonPDF(
  ...
  el.innerHTML = `
    <div style="text-align:center;width:100%;margin-bottom:28px;flex-shrink:0;">
      ...
      <p style="font-size:14px;color:#78716C;margin:0;">Document généré sur Aswilia · Registre Généalogique Comorien</p>
    </div>
  ...
  try {
    ...
    pdf.save(`Aswilia_${p.prenom}_${p.nom || ''}.pdf`);
  } finally {
